...
# 2. Carga de Datos Principales (Tabla General)
raw_data = {
    'Técnico': [
...
# 3. Datos del Calendario de la Semana (CSV Incrustado)
calendar_data = {
...
<style>
    ...
    .dashboard-footer { color: #a0a0a0; font-size: 10px; padding: 10px; text-align: center; margin-top: 30px;}
    ...
    .section-banner { background-color: transparent !important; border-bottom: 1px solid #333; margin-bottom: 10px; padding-bottom: 5px;}
</style>
"""
...
with st.container():
    ...
    with col1:
        st.markdown('<div style="color: white; font-weight: bold; font-size: 24px;">GYM <span style="font-size: 10px; vertical-align: super; color: #ff4b4b;">Shop</span><sup>®</sup></div><div style="color: #a0a0a0; font-size: 10px;">Bienestar, Salud & Felicidad</div><div style="color: white; font-size: 10px; margin-top: 5px;">CON MÁS DE <br> <span style="font-size: 16px; font-weight: bold;">30 AÑOS</span> <br> TRAYECTORIA</div>', unsafe_allow_html=True)
    with col2:
        st.markdown('<h1 style="text-align: center;">CUADRO TÉCNICOS SEMANAL</h1>', unsafe_allow_html=True)
    with col3:
        st.markdown('<div style="display: flex; align-items: center; justify-content: flex-end; color: #a0a0a0;"><div style="margin-right: 10px;">Techforce SOLUTIONS</div><div style="margin-right: 20px;">1 DE ABRIL, 2026</div></div>', unsafe_allow_html=True)
...
with col_left:
    with st.container():
        st.markdown('<div class="dashboard-panel"><h3>📈 RESUMEN TÉCNICOS</h3>', unsafe_allow_html=True)
        kpi_col1, kpi_col2, kpi_col3 = st.columns(3)
        with kpi_col1:
            st.markdown(f'<div class="metric-card-container"><div style="font-size: 48px; color: white; margin-bottom: 10px;">💳</div><div class="metric-label">TOTAL VENTAS REPORTADAS</div><div class="metric-value">{total_ventas}</div><div class="trend-positive">📈 +12% vs. mes anterior</div></div>', unsafe_allow_html=True)
        with kpi_col2:
            st.markdown(f'<div class="metric-card-container"><div style="font-size: 48px; color: white; margin-bottom: 10px;">💰</div><div class="metric-label">TOTAL GASTOS OPERATIVOS</div><div class="metric-value">COP {total_gastos:,.0f}</div><div class="trend-positive">📉 -5% vs. mes anterior</div></div>', unsafe_allow_html=True)
        with kpi_col3:
            st.markdown(f'<div class="metric-card-container"><div style="font-size: 48px; color: white; margin-bottom: 10px;">📊</div><div class="metric-label">% CUMPLIMIENTO PROMEDIO (Vs Agenda)</div><div class="metric-value">{cumplimiento_promedio:.1f}%</div><div class="trend-positive">📈 +3.1% vs. mes anterior</div></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

    with st.container():
        st.markdown('<div class="dashboard-panel" style="background-color: #1a1a1a;"><h3>📊 DETALLE DE OPERACIÓN POR TÉCNICO</h3>', unsafe_allow_html=True)
        columnas_originales = ['Técnico', 'Visitas registradas', 'Agenda semana pasada', '% de visitas vs agenda', 'Gastos', 'Cantidad de ventas reportada', 'Servicios agendados esta semana']
...
with col_right:
    with st.container():
        st.markdown('<div class="dashboard-panel"><h3>📊 COMPARATIVA: VISITAS REALES VS. AGENDADAS</h3>', unsafe_allow_html=True)
        df_chart = df.copy()
        ...
        fig_bar.update_layout(paper_bgcolor='#1a1a1a', plot_bgcolor='#1a1a1a', margin=dict(l=10, r=10, t=30, b=60), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, font=dict(color='white')), xaxis=dict(tickfont=dict(color='white', size=10), tickangle=-45, gridcolor='#333'), yaxis=dict(tickfont=dict(color='white'), gridcolor='#333', zerolinecolor='#333'))
...
# ==========================================
st.markdown('<div class="section-banner"><h3>🎫 TICKETS DE SOPORTE ZOHO DESK (Resumen de Backlog)</h3></div>', unsafe_allow_html=True)
...
# ==========================================
# SECCIÓN 3: CALENDARIO DE AGENDAMIENTOS
# ==========================================
st.markdown('<div class="section-banner"><h3>📅 CALENDARIO DE AGENDAMIENTOS (SEMANA ACTUAL)</h3></div>', unsafe_allow_html=True)
col_cal_left, col_cal_right = st.columns([1, 2])

with col_cal_left:
    with st.container():
        st.markdown('<div class="dashboard-panel"><h3>📊 Volumen Diario por Estado</h3>', unsafe_allow_html=True)
        cal_summary = df_cal.groupby(['Fecha', 'Estado']).size().reset_index(name='Total')
        color_map = {
            'CONFIRMADO CON CLIENTE': '#50e3c2', # Verde
            'POR CONFIRMAR CON CLIENTE': '#ff9f43', # Naranja
            'PENDIENTE PROGRAMAR': '#3b82f6' # Azul
        }
        fig_cal_bar = px.bar(cal_summary, x='Fecha', y='Total', color='Estado', color_discrete_map=color_map, text='Total', barmode='stack')
        fig_cal_bar.update_layout(paper_bgcolor='#1a1a1a', plot_bgcolor='#1a1a1a', margin=dict(l=10, r=10, t=30, b=30), legend=dict(orientation="h", yanchor="bottom", y=-0.3, xanchor="center", x=0.5, font=dict(color='white')), xaxis=dict(tickfont=dict(color='white'), gridcolor='#333', title=''), yaxis=dict(tickfont=dict(color='white'), gridcolor='#333', title='Visitas'))
        st.plotly_chart(fig_cal_bar, use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)

with col_cal_right:
    with st.container():
        st.markdown('<div class="dashboard-panel"><h3>📋 Detalle de Agenda por Técnico</h3>', unsafe_allow_html=True)
        
        # Procesar datos para la matriz tipo calendario
        state_map = {
            'CONFIRMADO CON CLIENTE': '✅ Conf.',
            'POR CONFIRMAR CON CLIENTE': '⏳ Por Conf.',
            'PENDIENTE PROGRAMAR': '🗓️ Pend.'
        }
        summary_mat = df_cal.groupby(['Técnico', 'Fecha', 'Estado']).size().reset_index(name='count')
        summary_mat['Estado_txt'] = summary_mat['count'].astype(str) + " " + summary_mat['Estado'].map(state_map)
        
        # Unir estados en caso de tener múltiples citas en un mismo día
        table_data = summary_mat.groupby(['Técnico', 'Fecha'])['Estado_txt'].apply(lambda x: ' | '.join(x)).reset_index()
        pivot_table = table_data.pivot(index='Técnico', columns='Fecha', values='Estado_txt').fillna('-')
        fechas_cols = sorted(pivot_table.columns.tolist())
        
        # Renderizar la tabla calendario
        fig_cal_table = go.Figure(data=[go.Table(
            header=dict(values=['Técnico'] + fechas_cols, fill_color='#1a1a1a', align='left', font=dict(color='white', size=12), line_color='#333'),
            cells=dict(
                values=[pivot_table.index] + [pivot_table[col] for col in fechas_cols],
                fill_color=['#1a1a1a', '#222'] * len(pivot_table), align='left', font=dict(color='#f0f0f0', size=11), line_color='#333'
            )
        )])
        fig_cal_table.update_layout(margin=dict(l=0, r=0, t=0, b=0), paper_bgcolor='#1a1a1a', plot_bgcolor='#1a1a1a', height=350)
        st.plotly_chart(fig_cal_table, use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)

# Footer
st.markdown('<div class="dashboard-footer">Nota: Los tickets Vencidos representan una falla crítica en el SLA (ITIL 4 best practice). <br>Generado por Techforce Data Analytics | Versión: 2026.01.05</div>', unsafe_allow_html=True)